...
export function UserApp() {
  ...
  useEffect(() => {
    const rawFrameId = searchParams.get('frame');
    if (rawFrameId) {
      setIsFrameLoading(true);
      const fetchFrame = async () => {
        try {
          const docRef = doc(db, 'shared_frames', rawFrameId);
  ...
  const generateVideo = async (src: string) => {
    ...
    setVideoUrl(null);
    try {
      const prompt = "A cinematic slow-motion video of a runner, high quality";
      const res = await fetch('/api/gemini', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({
      ...
      setAnalysis("Generando video con Veo AI...");
    } catch (err: any) {
      console.error(err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-stone-900 text-stone-100 p-6">
      <div className="max-w-7xl mx-auto space-y-8">
        ...
        {isFrameLoading && (
          <div className="fixed inset-0 z-[100] flex flex-col items-center justify-center bg-stone-950/80 backdrop-blur-xl animate-in fade-in duration-500">
            <div className="w-16 h-16 border-4 border-emerald-500/30 border-t-emerald-500 rounded-full animate-spin mb-4" />
            <p className="text-white font-bold tracking-tight">Cargando tu diseño exclusivo...</p>
            <p className="text-stone-400 text-xs mt-2">Estamos preparando todo para tu foto ✨</p>
          </div>
        )}
